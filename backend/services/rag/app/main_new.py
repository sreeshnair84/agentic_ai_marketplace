    ...
    try:
        async with async_session_maker() as session:
            ...
            result = await session.execute(text("""
                SELECT name, display_name, provider, api_endpoint, capabilities, pricing_info
                FROM llm_models 
                WHERE status = 'active' AND model_type = 'text-generation'
            """))
            ...
            result = await session.execute(text("""
                ...
                WHERE status = 'active' AND model_type = 'embedding'
            """))
